...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    let datos: Vec<i32> = input.read_int_vec(n);
    let mut sal = Vec::with_gen(n, |_| 0_usize);
    let ord = Vec::with_gen(n, |i| i);

    fn rec(ord: &[usize], datos: &[i32], mut k: i32, mut Li: usize, mut Ri: usize, sal: &mut [usize]) {
        let s = ord.len();
        if s == 0 { return; }
        if s == 1 {
            sal[ord[0]] = Li;
        ...
        let mut sup = Vec::new();
        for &u in ord {
            if datos[u] == -1 || datos[u] > k {
                sup.push(u);
            }
        }
        let ms = sup.len();
        let mr = s - ms;
        let (svL, _svR, rmL, _rmR) = if k & 1 == 1 {
            (Li, Li + ms - 1, Li + ms, Ri)
        } else {
            (Ri - ms + 1, Ri, Li, Ri - ms)
        };
        rec(&sup, datos, k + 1, svL, svL + ms - 1, sal);
        if mr == 0 { return; }
        let mut elim = Vec::with_gen(mr, |i| rmL + i);
        let mut pos = HashMap::with_capacity(s);
        ...
        while i < s {
            ...
            let mut j = i;
            while j < s && datos[ord[j]] == k {
                j += 1;
            }
            let sz = j - i;
            let mut bloq = elim[idx..idx + sz].to_vec();
            let tocL = pos[&ord[i]] == 0;
            let tocR = pos[&ord[j - 1]] == s - 1;
            if k & 1 == 1 {
                if tocL { bloq.reverse(); }
            } else {
                if tocR { bloq.reverse(); }
            }
            for t in 0..sz {
                sal[ord[i + t]] = bloq[t];
            }
