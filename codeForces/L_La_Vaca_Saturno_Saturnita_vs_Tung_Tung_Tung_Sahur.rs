...
const MODM: i64 = MOD - 1;
const TAM: usize = 33_000;
const MAXG: i32 = 32_967;
...
fn mod_cad(s: &str, m: i64) -> i64 {
    let mut res = 0;
    for b in s.bytes() {
        res = (res * 10 + (b - b'0') as i64) % m;
    }
    res
}

fn to_base(s: &str, b: u32) -> Vec<i32> {
    let mut n = BigUint::parse_bytes(s.as_bytes(), 10).unwrap();
    if n.is_zero() { return vec![0]; }
    let mut v = Vec::new();
    let bb = BigUint::from(b);
    while !n.is_zero() {
        let (q, r) = n.div_rem(&bb);
        v.push(r.to_u32().unwrap() as i32);
        n = q;
    }
    v.reverse();
    v
...
pub fn solve(inp: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    let b: u32 = inp.read();
    let l: String = inp.read();
    let r: String = inp.read();

    let dig_l = to_base(&l, b);
    let dig_r = to_base(&r, b);
    let kl = dig_l.len();
    let kr = dig_r.len();

    let mut alc = vec![false; TAM];
    for k in kl + 1..kr {
        let mx = k as i32 * (b as i32 - 1) - 1;
        for g in 0..=mx { alc[g as usize] = true; }
    }

    let mut marca = |k: usize, lo: &[i32], hi: &[i32]| {
        let mut dp = vec![vec![vec![false; TAM]; 2]; 2];
        dp[1][1][0] = true;
        for p in 0..k {
            let mut ndp = vec![vec![vec![false; TAM]; 2]; 2];
            for tl in 0..2 {
                for th in 0..2 {
                    for (s, &ok) in dp[tl][th].iter().enumerate().filter(|&(_, v)| *v) {
                        let mut l = if tl == 1 { lo[p] } else { 0 };
                        let h = if th == 1 { hi[p] } else { b as i32 - 1 };
                        if p == 0 && k > 1 { l = l.max(1); }
                        for d in l..=h {
                            let ntl = if tl == 1 && d == lo[p] { 1 } else { 0 };
                            let nth = if th == 1 && d == hi[p] { 1 } else { 0 };
                            if (s + d as usize) < TAM {
                                ndp[ntl][nth][s + d as usize] = true;
                            }
                        }
                    }
                }
        ...
        for tl in 0..2 {
            for th in 0..2 {
                for (s, &ok) in dp[tl][th].iter().enumerate().filter(|&(_, v)| *v) {
                    let g = k as i32 * (b as i32 - 1) - s as i32;
                    if (0..=MAXG).contains(&g) { alc[g as usize] = true; }
                }
            }
        }
    };

    let full_hi = vec![b as i32 - 1; kl];
    marca(kl, &dig_l, if kl == kr { &dig_r } else { &full_hi });
    if kr != kl {
        let full_lo = vec![0; kr];
        marca(kr, &full_lo, &dig_r);
    }

    let mut base = [0i32; 16];
    let mut rango = 0;
    for (g, &v) in alc.iter().enumerate().filter(|&(_, v)| *v) {
        let mut x = g as i32;
        for bit in (0..16).rev() {
    ...
    let rmod = mod_cad(&r, MODM);
    let lmod = mod_cad(&l, MODM);
    let mut nmod = (rmod - lmod).rem_euclid(MODM);
    nmod = (nmod + 1) % MODM;
    let e = (nmod - rango as i64).rem_euclid(MODM);
