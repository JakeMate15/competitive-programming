...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    let mut vx: i64 = input.read();
    let mut vy: i64 = input.read();
    if (vx, x) > (vy, y) {
    ...
    let vx2 = vx / g;
    let vy2 = vy / g;
    let (mut u, mut v) = (0_i64, 0_i64);
    exgcd(vy2, vx2, &mut u, &mut v);
    v = -v;
    u *= r;
    v *= r;
    let shift = 2_000_000_000;
    u += vx2 * shift;
    v += vy2 * shift;
    let t = std::cmp::min((u - 1) / vx2, (v - 1) / vy2);
    u -= vx2 * t;
    v -= vy2 * t;
    let mut ans = u + v - 2;
    ans += (u + v) / 2;
