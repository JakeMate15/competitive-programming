...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    let sb = input.next_token().unwrap();
    let tb = input.next_token().unwrap();
    let s = String::from_utf8(sb).unwrap();
    let mut t: Vec<char> = String::from_utf8(tb).unwrap().chars().collect();
    t.reverse();
    let n = s.len();
    let m = t.len();

    let mut ed = Vec::with_gen(n, |_| Vec::new());

    for ch1 in 0..6_usize {
        let mut A = vec![0.0; n];
        for (i, c) in s.chars().enumerate() {
            if (c as u8 - b'a') as usize == ch1 {
                A[i] = 1.0;
            }
        }
        for ch2 in 0..6_usize {
            if ch1 == ch2 { continue; }
            let mut Bv = vec![0.0; m];
            for (i, &c) in t.iter().enumerate() {
                if (c as u8 - b'a') as usize == ch2 {
                    Bv[i] = 1.0;
                }
            }
            let Cc = conv(&A, &Bv);
