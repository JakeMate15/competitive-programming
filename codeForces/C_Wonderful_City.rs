...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    for _ in 0..n * n {
        h.push(input.read_long());
    }
    let a = input.read_long_vec(n);
    let b = input.read_long_vec(n);

    let msk = |hor: bool| -> Vec<i32> {
        let m = n - 1;
    ...
    let mc = msk(true);
    let mr = msk(false);
    if mc.is_empty() || mr.is_empty() {
        out.print_line(-1);
        return;
    }

    let go = |c: &Vec<i64>, m: &Vec<i32>| -> i64 {
        let s = c.len();
        ...
        for p in (0..s).rev() {
            for ant in 0..2 {
                let mut best = INF;
                for cur in 0..2 {
                    if p == 0 || (m[p - 1] & (1 << (ant * 2 + cur))) != 0 {
                        let sub = dp[p + 1][cur];
                        if sub < INF {
                            let cost = sub + if cur == 1 { c[p] } else { 0 };
                            if cost < best {
                                best = cost;
                            }
                        }
        ...
        for f in 0..2 {
            let sub = dp[1][f];
            if sub < INF {
                let cost = sub + if f == 1 { c[0] } else { 0 };
                if cost < res {
                    res = cost;
                }
            }
