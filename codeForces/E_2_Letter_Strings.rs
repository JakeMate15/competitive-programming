...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    let mut nc: HashMap<String, i64> = HashMap::new();
    let mut res = 0_i64;
    for _ in 0..n {
        let tok = input.next_token().unwrap();
        let s = String::from_utf8(tok).unwrap();
        for j in 0..2 {
            for b in b'a'..=b'k' {
                let mut chars: Vec<char> = s.chars().collect();
                chars[j] = b as char;
                let aux: String = chars.iter().collect();
                if aux != s {
                    if let Some(&cnt) = nc.get(&aux) {
                        res += cnt;
                    }
                }
            }
        }
        *nc.entry(s).or_insert(0) += 1;
    }
    out.print_line(res);
}
