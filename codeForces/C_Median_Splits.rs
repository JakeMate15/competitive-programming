...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    for i in 1..=n {
        a[i] = input.read_size() as i64;
        b[i] = a[i] <= k;
    }
    let mut pref = vec![false; n + 2];
    let mut uno = 0;
    for l in 1..=n {
        if b[l] { uno += 1; }
        if 2 * uno >= (l + (l & 1)) as i32 {
            pref[l] = true;
        }
    }
    let mut suf = vec![false; n + 2];
    uno = 0;
    for s in (1..=n).rev() {
        if b[s] { uno += 1; }
        let len = (n - s + 1) as i32;
        if 2 * uno >= (len + (len & 1)) {
            suf[s] = true;
        }
    }
    ...
    for i in 1..=n {
        f[i] = f[i - 1] + if b[i] { 1 } else { -1 };
    }
    let mut mn = [0_i32, std::i32::MAX];
    let mut fin = vec![false; n + 2];
    for r in 1..=n {
        let pr = (r & 1) as usize;
        if mn[pr] <= f[r] { fin[r] = true; }
        if mn[pr ^ 1] <= f[r] - 1 { fin[r] = true; }
        mn[pr] = mn[pr].min(f[r]);
    }
    let mut f2 = vec![0_i32; n + 1];
    for i in 1..=n {
        let v = if b[n - i + 1] { 1 } else { -1 };
        f2[i] = f2[i - 1] + v;
    }
    let mut mn2 = [0_i32, std::i32::MAX];
    let mut endr = vec![false; n + 2];
    for r in 1..=n {
        let pr = (r & 1) as usize;
        if mn2[pr] <= f2[r] { endr[r] = true; }
        if mn2[pr ^ 1] <= f2[r] - 1 { endr[r] = true; }
        mn2[pr] = mn2[pr].min(f2[r]);
    }
    let mut ini = vec![false; n + 2];
    for p in 1..=n {
        ini[p] = endr[n - p + 1];
    }
    let mut ok = false;
