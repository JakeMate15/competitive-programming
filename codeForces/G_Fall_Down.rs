...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    for _ in 0..n {
        let row = input.next_token().unwrap();
        grid.extend(row);
    }
    let mut i = (n as i32) - 1;
    while i >= 0 {
        for j in 0..m {
            let idx = (i as usize) * m + j;
            if grid[idx] == b'*' {
                let mut ii = i;
                while ii + 1 < n as i32 && grid[((ii + 1) as usize) * m + j] == b'.' {
                    grid[(ii as usize) * m + j] = b'.';
                    grid[((ii + 1) as usize) * m + j] = b'*';
                    ii += 1;
                }
                i = ii;
            }
