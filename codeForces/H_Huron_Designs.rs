...
pub fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    for _ in 0..n {
        ...
        p.push(input.read::<i64>());
        c.push(input.read::<i64>());
        lx.push(input.read::<i64>());
        rx.push(input.read::<i64>());
        ly.push(input.read::<i64>());
        ry.push(input.read::<i64>());
    }
    let mut avgx = Vec::with_capacity(n);
    for i in 0..n {
        avgx.push((lx[i] + rx[i]) as f64 * 0.5);
    }
    let N = 1 << n;
    let mut dp: Vec<Vec<(i64, f64)>> = vec![Vec::new(); N];
    dp[0].push((0, 0.0));
    let mut ans = 0.0;
    for mask in 0..N {
        let states = dp[mask].clone();
        for &(st_t, st_v) in &states {
            for j in 0..n {
                if mask & (1 << j) == 0 {
                    let nt = st_t + c[j];
                    if nt > d[j] { continue; }
                    let prob = if nt <= ly[j] {
                        1.0
                    } else if nt >= ry[j] || ry[j] == ly[j] {
                        0.0
                    } else {
                        let pr = (ry[j] - nt) as f64 / (ry[j] - ly[j]) as f64;
                        if pr < 0.0 { 0.0 } else { pr }
                    };
                    let nv = st_v + p[j] as f64 + avgx[j] * prob;
                    let nmask = mask | (1 << j);
                    let to = &mut dp[nmask];
