...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    let mask = ((1 << k) - 1) as i32;

    let mut dp: Vec<Vec<(i32, i32)>> = Vec::with_gen(n, |_| Vec::new());
    let mut act: Vec<i32> = Vec::new();
    let mut mar = vec![-1_i32; (mask as usize) + 1];
    let mut res = Vec::with_gen(n, |_| 0_i32);
    let mut it = 0_i32;

    let mut get = |mut i: usize, mut s: i32| {
        let mut pila: Vec<(usize, i32)> = Vec::new();
        let mut r;
        loop {
            if let Some(&(_, v)) = dp[i].iter().find(|&&(x, _)| x == s) {
                r = v;
                break;
            }
            pila.push((i, s));
        ...
        while let Some((pi, ps)) = pila.pop() {
            let nxt = if pi + 1 == n {
                ps
            } else {
                let ns = (!(ps | a[pi + 1])) & mask;
                dp[pi + 1]
                    .iter()
                    .find(|&&(x, _)| x == ns)
                    .unwrap()
