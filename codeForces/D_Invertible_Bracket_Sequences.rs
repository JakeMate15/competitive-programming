...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut PreCalc) {
    ...
    let mut mx = 0;
    for i in 0..n {
        let v = if s[i] == b'(' { 1 } else { -1 };
        let cur = pref[i] + v;
        mx = max(mx, cur);
        pref.push(cur);
    }
    let mut cnt: Vec<Vec<usize>> = vec![Vec::new(); (mx + 1) as usize];
    for i in 0..=n {
        cnt[pref[i] as usize].push(i);
    }
    let st = SegmentTree::new(pref.clone(), i32::MIN, |a, b| max(*a, *b));
    let mut ans: i64 = 0;
    for i in 1..=n {
        let base = pref[i - 1];
        let mut lo = i;
        ...
        while lo + 1 < hi {
            let mid = (lo + hi) >> 1;
            if 2 * base >= st.query(i, mid) {
                lo = mid;
            } else {
                hi = mid;
            }
        }
        let v = &cnt[base as usize];
        let lpos = v.binary_search(&i).unwrap_or_else(|x| x);
        let hi_pos = v.binary_search(&(lo + 1)).unwrap_or_else(|x| x);
        ans += (hi_pos - lpos) as i64;
    }
    out.print_line(ans);
}
