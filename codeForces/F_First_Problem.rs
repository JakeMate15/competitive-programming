...
impl SqrtDecomp {
    fn new(n: usize, init: Vec<i32>) -> Self {
        let mut b = (n as f64).sqrt() as usize;
        if b * b < n {
            b += 1;
        }
        let nb = (n + b - 1) / b;
        let mut sd = SqrtDecomp {
            n,
            b,
            nb,
            a: init,
    ...
    fn rebuild_block(&mut self, bi: usize) {
        let l = bi * self.b;
        let r = min(self.n, l + self.b);
        let mut mx = i32::MIN;
        for i in l..r {
            mx = mx.max(self.a[i]);
        }
        self.block_max[bi] = mx;
    }
    ...
    fn range_add(&mut self, l: usize, r: usize, v: i32) {
        ...
        if bl == br {
            ...
            self.rebuild_block(bl);
        } else {
            let endl = (bl + 1) * self.b - 1;
            for i in l..=endl {
                self.a[i] += v;
            }
            self.rebuild_block(bl);
            ...
            for i in br * self.b..=r {
                self.a[i] += v;
            }
            self.rebuild_block(br);
        }
    }
    ...
    fn range_max(&self, l: usize, r: usize) -> i32 {
        ...
        if bl == br {
            for i in l..=r {
                ans = ans.max(self.a[i] + self.add[bl]);
            }
        } else {
            let endl = (bl + 1) * self.b - 1;
            for i in l..=endl {
            ...
            for bi in bl+1..br {
                ans = ans.max(self.block_max[bi] + self.add[bi]);
            }
            for i in br * self.b..=r {
                ans = ans.max(self.a[i] + self.add[br]);
            }
        }
        ans
    }

    fn global_max(&self) -> i32 {
        let mut gm = i32::MIN;
        for bi in 0..self.nb {
            gm = gm.max(self.block_max[bi] + self.add[bi]);
        }
        gm
    }

    fn range_reset_max(&mut self, l: usize, r: usize) {
        let gm = self.global_max();
        let bl = l / self.b;
        let br = r / self.b;
        if bl == br {
            for i in l..=r {
                if self.a[i] + self.add[bl] == gm {
                    self.a[i] = -self.add[bl];
                }
            }
            self.rebuild_block(bl);
        } else {
            ...
            self.rebuild_block(bl);
            for bi in bl+1..br {
                if self.block_max[bi] + self.add[bi] < gm {
                    continue;
                }
                let start = bi * self.b;
                let endb = min(self.n, start + self.b);
                for i in start..endb {
                    if self.a[i] + self.add[bi] == gm {
                        self.a[i] = -self.add[bi];
                    }
                }
                self.rebuild_block(bi);
            }
            for i in br * self.b..=r {
                if self.a[i] + self.add[br] == gm {
                    self.a[i] = -self.add[br];
                }
            }
            self.rebuild_block(br);
        }
...
pub fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    for _ in 0..n {
        init.push(input.read::<i32>());
    }
    let mut sd = SqrtDecomp::new(n, init);
    for _ in 0..k {
        let op = input.read_char() as char;
        let l = input.read::<usize>() - 1;
        let r = input.read::<usize>() - 1;
        if op == 'Q' {
            out.print_line(sd.range_max(l, r));
        } else if op == 'A' {
            sd.range_add(l, r, 1);
        } else if op == 'R' {
            sd.range_reset_max(l, r);
        }
    }
}
