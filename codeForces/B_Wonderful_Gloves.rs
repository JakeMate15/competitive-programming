...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    let l: Vec<i64> = input.read_long_vec(n);
    let r: Vec<i64> = input.read_long_vec(n);

    let mn: Vec<i64> = Vec::with_gen(n, |i| l[i].min(r[i]));
    let mut mx: Vec<i64> = Vec::with_gen(n, |i| l[i].max(r[i]));
    let s_max: i64 = mx.iter().sum();

    let mut mn_sorted = mn;
    mn_sorted.sort_unstable_by(|a, b| b.cmp(a));

    let s_e: i64 = mn_sorted.into_iter().take(k.saturating_sub(1)).sum();
    out.print_line(s_max + s_e + 1);
}
