...
pub fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    let mut ci = vec![0; n + 1];
    let mut mp: HashMap<i64, Vec<usize>> = HashMap::new();
    ...
    while let Some(&u) = st.last() {
        if ci[u] == 0 {
            tm += 1;
            ti[u] = tm;
            mp.entry(k[u]).or_default().push(tm);
        }
        if ci[u] < h[u].len() {
            let v = h[u][ci[u]];
            ci[u] += 1;
    ...
    for _ in 0..q {
        let x: usize = input.read();
        let kk: i64 = input.read();
        if let Some(v) = mp.get(&kk) {
            let l = v.partition_point(|&p| p < ti[x]);
            let r = v.partition_point(|&p| p <= to[x]);
            out.print_line(r - l);
        } else {
            out.print_line(0);
        }
