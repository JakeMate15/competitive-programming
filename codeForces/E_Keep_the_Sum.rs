...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    let s = input.read_int() as i32;
    let mut x: Vec<i32> = input.read_int_vec(n);

    let mut order: Vec<usize> = (0..n).collect();
    order.sort_unstable_by_key(|&i| x[i]);
    ...
    while g < h as usize {
        while g < h as usize && x[order[g]] + x[order[h as usize]] > s {
            h -= 1;
        }
        if g < h as usize && x[order[g]] + x[order[h as usize]] == s {
            u = order[g];
            v = order[h as usize];
            break;
        }
    ...
    if u == usize::MAX {
        let ok = x.windows(2).all(|w| w[0] <= w[1]);
        out.print_line(if ok { 0 } else { -1 });
        return;
    }
    ...
    let mut ops: Vec<(usize, usize, i32)> = Vec::new();
    fn apply(x: &mut [i32], ops: &mut Vec<(usize, usize, i32)>, iu: usize, iv: usize, d: i32) {
        if d == 0 {
            return;
        }
        x[iu] -= d;
    ...
    loop {
        if u_idx != 0 && u_idx != n - 1 {
            let i = pos[u_idx];
            let d = x[u_idx] - x[i];
            apply(&mut x, &mut ops, u_idx, v_idx, d);
            x[v_idx] = s - x[u_idx];
            aa[u_idx] = u_idx;
            aa[i] = 0;
            pos[u_idx] = u_idx;
            pos[0] = i;
            u_idx = i;
        } else if v_idx != 0 && v_idx != n - 1 {
            let i = pos[v_idx];
            let d = x[v_idx] - x[i];
            apply(&mut x, &mut ops, v_idx, u_idx, d);
            x[u_idx] = s - x[v_idx];
            aa[v_idx] = v_idx;
            aa[i] = n - 1;
            pos[v_idx] = v_idx;
    ...
    let final_d = x[0];
    apply(&mut x, &mut ops, 0, n - 1, final_d);
