...
pub fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    let mut v: Vec<i64> = input.read_vec(c);
    let f: usize = input.read();
    let m: Vec<i64> = input.read_vec(f * c);
    let mut d: i64 = 1_000_000_000_000_000_000;
    let mut o: i64 = 1_000_000_000;
    for i in 0..c {
        let suma: i64 = (0..f).map(|j| m[j * c + i]).sum();
        if suma != 0 {
            let dia = 1 + v[i] / suma;
            if dia <= d {
                if dia < d {
