...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    let a_bytes = input.next_token().unwrap();
    let b_bytes = input.next_token().unwrap();
    let na = a_bytes.len();
    let nb = b_bytes.len();
    ...
    while i < na || j < nb {
        if i < na {
            ...
            while i < na && a_bytes[i] == curr {
                aux.push(curr as char);
                i += 1;
            }
            if !aux.is_empty() {
                A.push(aux);
    ...
    for k in 0..A.len() {
        let ak = &A[k];
        let bk = &B[k];
        let ca = ak.as_bytes()[0];
        if ca != bk.as_bytes()[0] || ak.len() * 2 < bk.len() || ak.len() > bk.len() {
            out.print_line("NO");
            return;
        }
    }
