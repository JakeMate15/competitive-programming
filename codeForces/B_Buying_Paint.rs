...
pub fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    let mut m: Vec<i64> = input.read_vec(n * n);
    let mut ans: i64 = 0;
    let df = [1isize, 0, -1, 0];
    let dc = [0isize, 1, 0, -1];
    let obt = |t: &Vec<i64>| -> i64 {
        ...
        let mut d = 0usize;
        while seq.len() < n * n {
            let idx = (f as usize) * n + c as usize;
            vis[idx] = true;
            seq.push(t[idx]);
            if seq.len() == n * n { break; }
            let mut nf = f + df[d];
            let mut nc = c + dc[d];
            let oi = nf < 0 || nf >= n as isize || nc < 0 || nc >= n as isize
                  || vis[(nf as usize) * n + (nc as usize)];
            if oi {
                d = (d + 1) & 3;
                nf = f + df[d];
                nc = c + dc[d];
