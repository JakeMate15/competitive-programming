...
fn solve(input: &mut Input, out: &mut Output, _tc: usize, _data: &mut ()) {
    ...
    let mut p = Vec::with_gen(n + 1, |_| 0);
    let mut costo = Vec::with_gen(n + 1, |_| 0);
    for i in 1..=n {
        p[i] = input.read_size();
    }
    for i in 1..=n {
        costo[i] = input.read_size();
    }
    let mut g = Vec::with_gen(2 * n, |_| Edge { cost: 0, u: 0, v: 0 });
    let mut gi = 0;
    for i in 1..=n {
        g[gi] = Edge { cost: -2 * (costo[i] as i64), u: i, v: p[i] };
        gi += 1;
        g[gi] = Edge { cost: -(costo[i] as i64), u: p[i], v: i };
        gi += 1;
    }
    g.truncate(gi);
    g.sort_unstable_by(|a, b| a.cost.cmp(&b.cost));
    let mut tree = Vec::with_gen(n + 1, |_| Vec::new());
    let mut indeg = Vec::with_gen(n + 1, |_| 0);
